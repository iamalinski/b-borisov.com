...
@section('title', 'Редактиране на пакет')
@section('breadcrumb', 'Редактиране на пакет')

@section('header', 'Редактиране на пакет')
@section('subheader', 'Раздел: ' . $package->section->name)
...
<script>
    ...
    function addItem(isExtra = false, title = '') {
    ...
    addItem(false, @json($item->title));
    @endforeach

    @foreach($package->extraItems as $item)
    addItem(true, @json($item->title));
    @endforeach

    // Add one empty item if there are no standard items
    @if($package->standardItems->count() === 0)
    addItem(false);
    @endif
</script>
@endpush
